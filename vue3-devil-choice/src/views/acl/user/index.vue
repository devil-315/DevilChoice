    ...
    <el-table @selection-change="selectChange" style="margin: 10px 0px" border :data="userArr">
      ...
      <el-table-column label="操作" width="300px" align="center">
        <template #="{row,$index}">
          ...
          <el-popconfirm :title="`你确定要删除${row.username}嘛~`" width="300px" @confirm="deleteUser(row.id)">
            <template #reference>
...
const save =async () => {
  ...
  //添加或者更新
  let result = await reqAddOrUpdateUser((userParams as User))
  if(result.code == 200){
    //关闭抽屉
    draw.value = false
    //提示消息
    ...
    //获取最新的全部账号信息
    getHasUser(userParams.id ? pageNo.value : pageNo.value - 1)
    //浏览器自动刷新一次
...
const deleteSelectUser =async () => {
  //整理批量删除的数据
  let idList:any = selectIdArr.value.map(item => (item.id as number))
  //批量删除的请求
